package mysql

import (
    "context"
    "database/sql"
    "errors"
    "io/ioutil"
    "log"

    "wait4it/pkg/model"

    "github.com/go-sql-driver/mysql"
)

// BuildContext updates the MySQLConnection object with information from the check context.
func (m *MySQLConnection) BuildContext(cx model.CheckContext) {
    m.Port = cx.Port
    m.Host = cx.Host
    m.Username = cx.Username
    m.Password = cx.Password
    m.DatabaseName = cx.DatabaseName
}

// Validate checks the MySQLConnection object for any configuration errors.
func (m *MySQLConnection) Validate() error {
    if len(m.Host) == 0 || len(m.Username) == 0 {
        return errors.New("host or username can't be empty")
    }

    if m.Port < 1 || m.Port > 65535 {
        return errors.New("invalid port range for mysql")
    }

    return nil
}

// Check performs the actual connection attempt to the MySQL server.
func (m *MySQLConnection) Check(ctx context.Context) (bool, bool, error) {
    dsl := m.BuildConnectionString()

    db, err := sql.Open("mysql", dsl)
    if err != nil {
        return false, true, err
    }

    err = mysql.SetLogger(log.New(ioutil.Discard, "", log.LstdFlags))
    if err != nil {
        return false, true, err
    }

    err = db.PingContext(ctx)
    if err != nil {
        // todo: need a logger
        return false, false, nil
    }
    _ = db.Close()

    return true, true, nil
}
